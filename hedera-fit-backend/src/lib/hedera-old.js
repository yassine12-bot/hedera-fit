delete require.cache[require.resolve('./hedera')]; // Force reload
const {
  Client,
  AccountId,
  PrivateKey,
...
class HederaService {
   ...
   * Note: Les métadonnées Hedera sont limitées à 100 bytes
   * On stocke seulement un ID minimal on-chain
   * Les vraies métadonnées (nom, description, image) sont dans la DB locale
   * 
   * IMPORTANT: Pour l'instant, les badges restent dans ton wallet principal
   * Plus tard, quand chaque user aura son wallet Hedera, on pourra les transférer
   */
  async mintBadge(recipientAccountId, badgeType) {
    try {
      ...
      // Métadonnées minimales (< 100 bytes)
      // Format: "TYPE:YYYYMMDD" (ex: "ROOKIE:20250102")
      ...
      // ⭐ AJOUTE CES 3 LIGNES
console.log('========================================');
console.log('⭐⭐⭐ VERSION MODIFIÉE - PAS DE TRANSFER ⭐⭐⭐');
console.log('========================================');

console.log('✅ Badge créé et conservé dans le trésor (wallet principal)');

      // ✅ MODIFIÉ: On garde le badge dans le trésor (ton wallet principal)
      // Pas de transfer pour éviter ACCOUNT_REPEATED_IN_ACCOUNT_AMOUNTS
      console.log('✅ Badge créé et conservé dans le trésor (wallet principal)');
